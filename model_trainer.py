...
def generate_raw_data(train_size=60000):
    ...
    print("::util.generate_raw_data:: Generated {} rows of training data and {} rows of eval data.".format(train_size,
                                                                                                           eval_size))
...
def get_new_model_candidates_from_wb(project, model_use_case_id, metric_key):
    ...
    print("::util.filter_model_candidates:: Total unevaluated models on {}: {}/{}".format(metric_key,
                                                                                          len(unevaluated_candidates),
...
def promote_best_model_in_wb(project, model_use_case_id, metric):
    ...
    if (best_model is None):
        ...
        if ('production' in best_model.aliases):
            print("::util.promote_best_model_in_wb:: Existing production model {} has best score {} on {}".format(
                best_model.name, best_loss, metric, metric))
        else:
            print("::util.promote_best_model_in_wb:: Promoting model {} with best score {} on {} to production".format(
                best_model.name, best_loss, metric, metric))
...
x_train, y_train = download_training_dataset_from_wb(model_use_case_id)

# Then we train a model using this data. For simplicity, we use a sequential model.
model = build_and_train_model(x_train, y_train, config=run.config)
...
publish_model_candidate_to_wb(model, model_use_case_id)
